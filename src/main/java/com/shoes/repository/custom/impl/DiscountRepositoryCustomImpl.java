...
public class DiscountRepositoryCustomImpl implements DiscountRepositoryCustom {
    ...
    Query buildQuery(String searchText) {
        ...
        StringBuilder sql = new StringBuilder(
            ...
            "    case\n" +
            "    \twhen d.status = 1 and now() < d.start_date then 'Chưa áp dụng'\n" +
            "    \twhen d.status = 1 and now() >= d.start_date and d.end_date >= now() then 'Đang áp dụng'\n" +
            "    \twhen d.status = 1 and now() > d.end_date  then 'Hết hạn'\n" +
            "    end as status,\n" +
